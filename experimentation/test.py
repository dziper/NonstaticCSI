...
for x in range(450):
    ...
    ant1_subcarrier1_csi.append(csi[1][1])
print(np.array(ant1_subcarrier1_csi).shape)

real_parts = [np.abs(num) for num in ant1_subcarrier1_csi]
complex_parts = [np.angle(num) for num in ant1_subcarrier1_csi]
# real_parts = [abs(num) for num in ant1_subcarrier1_csi]
# complex_parts = [np.angle(num) for num in ant1_subcarrier1_csi]

import matplotlib.pyplot as plt
import pandas as pd
train_size = int(0.99 * len(real_parts)) # can change if too big
...
trend = np.arange(450)
seasonality = np.array(complex_parts)
series = np.unwrap(seasonality)
print(series)
plot_acf(series)
plt.show() 
